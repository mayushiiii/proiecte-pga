...
void Lab10::Init()
{
	...
	LoadShader("Normal", "WorldSpace", "Color", "Normal");

    m_instances = 1;

	m_start_time = 2.0f;
	m_animation_time = 3.0f;
...
void Lab10::Update(float deltaTimeSeconds)
{
    ...
    {
        if (m_wait_time < m_start_time && !m_start_animation)
        {
            m_wait_time += deltaTimeSeconds;
        }
        else
        {
            m_start_animation = true;
        }

        if (m_explosion_time < m_animation_time && m_start_animation)
        {
            m_explosion_time += deltaTimeSeconds;
        }
...
void Lab10::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture)
{
	...
	int loc_instances = glGetUniformLocation(shader->program, "instances");
	glUniform1i(loc_instances, m_instances);

    if (m_explosion_time < m_animation_time && m_start_animation)
    {
	    int loc_time = glGetUniformLocation(shader->program, "time");
	    glUniform1f(loc_time, m_explosion_time);
    }
	else if (m_explosion_time >= m_animation_time && m_start_animation)
    {
        int loc_time = glGetUniformLocation(shader->program, "time");
        glUniform1f(loc_time, 0.0f);
        m_wait_time = 0.0f;
        m_start_animation = false;
		m_explosion_time = 0.0f;
    }

	int loc_amplitude = glGetUniformLocation(shader->program, "amplitude");
	glUniform1f(loc_amplitude, m_amplitude);
